    ...
    class ConvolutionalNetwork(nn.Module):
        def __init__(self):
            ...
            self.GlyphA = nn.Sequential(
                ...
                nn.Dropout(p=0.1)
                nn.view(-1, 47*47*16)
                nn.Linear(47*47*16, 180),
                nn.ReLU(),
                nn.Linear(180, 120),
                nn.ReLU(),
                nn.Linear(120, 1138)
            )
            self.GlyphB = nn.Sequential(
                nn.Conv2d(3, 6, 3, 1),
                nn.ReLU(),
                nn.avg_pool2d(X, 2, 2),
                nn.Dropout(p=0.2)
        ...
        def forward(self, X):
            #Define Multi-Head Outputs
            return {
                'A': F.log_softmax(self.GlyphA, dim=1),
                'B': F.log_softmax(self.GlyphB, dim=1),
                'C': F.log_softmax(self.GlyphC, dim=1),
                'D': F.log_softmax(self.GlyphD, dim=1),
                'E': F.log_softmax(self.GlyphE, dim=1)
            }
    ...
    if choice == "1":
        ...
        for i in range(epochs):
            ...
            for b, (X_train, y_train) in enumerate(train_loader):
                ...
                # Apply the model
                #########################################
                #New loss function using multiple outputs
                indie_loss = []
                indie_loss = MAiAn(X_train)  # we don't flatten X-train here
                
                #Sum the multi-head losses
                y_pred = 0
                for i in range(0,4):
                    y_pred = y_pred + indie_loss[i]
